...
(deftemplate selected-value (slot amount))
...
(defrule Process
	...
	(selected-value (amount ?givenAmount&~:(>= ?num ?givenAmount)))
	=>
	(printout t crlf "Needed amount: R" ?givenAmount crlf "	Add a R5,R2,R1,50c,20c,10c:   --> Current amount: R" ?num crlf ":>> ")
	(bind ?ans (readline))
		(retract ?fact)					
...
(defrule End
	...
	(selected-value (amount ?givenAmount&:(>= ?total ?givenAmount)))
	=>
	(bind ?leftover (- ?total ?givenAmount))
